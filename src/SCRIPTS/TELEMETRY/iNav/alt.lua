local function view(data, config, modes, dir, units, labels, gpsDegMin, hdopGraph, icons, calcBearing, calcDir, VERSION, SMLCD, FLASH, FILE_PATH, text, line, rect, fill, frmt)
	...
	local gpsFlags = SMLSIZE + RIGHT + ((not data.telem or not data.gpsFix) and FLASH or WHITE)
	local tmp, pitch
	...
	if SMLCD then
		if data.showDir then
			...
			text(LEFT_POS + 10, 49, "Dist", SMLSIZE + iNavZone.options.Text)
			icons.home(LEFT_POS + 2, 49)
			-- Altitude
			tmp = data.showMax and data.altitudeMax or data.altitude
			text(RIGHT_POS, 57, math.floor(tmp + 0.5) .. units[data.alt_unit], SMLSIZE + RIGHT + ((not data.telem or tmp + 0.5 >= config[6].v) and FLASH or iNavZone.options.Text))
			text(RIGHT_POS, 49, "Alt", SMLSIZE + RIGHT +iNavZone.options.Text)
	...
	if data.startup == 0 then
		...
		if not SMLCD then
			text(RIGHT_POS - 60, 19, math.floor(data.altMax + 0.5) .. units[data.alt_unit], SMLSIZE + RIGHT + iNavZone.options.Text)
		end

		-- Orientation
		if not SMLCD and data.telem then
			if data.showDir or data.headingRef == -1 then
				text(LEFT_POS + 12, 29, dir[0], SMLSIZE + iNavZone.options.Text)
	...
	if not SMLCD then
		...
		text(LEFT_DIV + 2, 30, "Dist", SMLSIZEiNavZone.options.Text)
